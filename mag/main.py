...
from train_process import test, train,gmlp_train, gmlp_train_sle
...
def run(args, data, subset_list, device, stage=0):
    ...
    feats, label_emb, teacher_probs, enhance_idx, labels, in_feats, n_classes, \
    ...
    if stage > 0:
        enhance_loader = torch.utils.data.DataLoader(
            enhance_idx.cpu(), batch_size=int(args.batch_size*len(enhance_idx)/(len(train_nid)+len(enhance_idx))), shuffle=True, drop_last=False, num_workers=2)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if stage > 0:
            if args.model=="NARS_gmlp":
                #gmlp_train(model, feats, label_emb, teacher_probs, labels_with_pseudos, loss_fcn, optimizer, train_loader_with_pseudos,epoch,num_epochs )
                gmlp_train_sle(model, feats, label_emb, teacher_probs, labels, loss_fcn, optimizer, train_loader, enhance_loader, epoch, num_epochs )
            else:
                #train(model, feats, label_emb, teacher_probs, labels_with_pseudos, loss_fcn, optimizer, train_loader_with_pseudos )
                train(model, feats, label_emb, teacher_probs, labels, loss_fcn, optimizer, train_loader)
        else:
            if args.model=="NARS_gmlp":
                #train(model, feats, label_emb, teacher_probs, labels, loss_fcn, optimizer, train_loader )
                #gmlp_train(model, feats, label_emb, teacher_probs, labels_with_pseudos, loss_fcn, optimizer, train_loader_with_pseudos,epoch,num_epochs )
                gmlp_train(model, feats, label_emb, teacher_probs, labels, loss_fcn, optimizer, train_loader, epoch, num_epochs )
            else:
                #train(model, feats, label_emb, teacher_probs, labels_with_pseudos, loss_fcn, optimizer, train_loader_with_pseudos )
                train(model, feats, label_emb, teacher_probs, labels, loss_fcn, optimizer, train_loader)
        med = time.time()
    ...
    if stage > 0:
        ensemble_prob = probs
        teacher_probs = torch.zeros(probs.shape[0], probs.shape[1])
        ...
        ensemble_prob += teacher_probs
        for i in range(1, stage):
            ensemble_prob += torch.load(f'./predict_prob_{i}.pt')
        ensemble_prob /= (stage+1)
        train_acc = (torch.argmax(ensemble_prob[:len(train_nid)], dim=1) == labels[train_nid].cpu()).float().sum() / len(train_nid)
        valid_acc = (torch.argmax(ensemble_prob[len(train_nid):len(train_nid)+len(val_nid)], dim=1) == labels[val_nid].cpu()).float().sum() / len(val_nid)
        test_acc = (torch.argmax(ensemble_prob[len(train_nid)+len(val_nid):len(train_nid)+len(val_nid)+len(test_nid)], dim=1) == labels[test_nid].cpu()).float().sum() / len(test_nid)
        print("Ensemble model Train ACC is {}".format(train_acc))
        print("Ensemble model Valid ACC is {}".format(valid_acc))
        print("Ensemble model Test ACC is {}".format(test_acc))


    del model, best_model
    del feats, label_emb, teacher_probs, labels
    gc.collect()
...
def main(args):
    ...
    for i in range(args.num_runs):
        ...
        for stage in range(args.warm_start, len(args.epoch_setting)):
            ...
            torch.save(probs, f'./predict_prob_{stage}.pt')
